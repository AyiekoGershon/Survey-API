...
async def submit_response(survey_id: int, request: Request) -> Response:
    ...
    try:
        ...
        question_map = {q[1]: {"id": q[0], "type": q[2]} for q in questions}
        ...
        for key, value in form.multi_items():
            ...
            # Handle file uploads (check using hasattr for robustness)
            if _is_file_upload(value):
                if value.filename:
                    ...
                    file_path = os.path.join(response_dir, value.filename)
                    ...
                    cursor.execute(
                        """INSERT INTO certificates (response_id, filename, file_path)
                           VALUES (%s, %s, %s)""",
                        (response_id, value.filename, file_path)
                    )
                    has_any_answer = True
...
async def get_responses(
    ...
    try:
        ...
        responses_dict["question_responses"].update(response_entries)
